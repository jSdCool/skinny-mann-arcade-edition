...
String Menue ="creds"/*,level="n"*/,version="0.6.2_Early_Access",ip="localhost",name="can't_be_botherd_to_chane_it",input,outher_name,file_path,rootPath,stageType="",settingsMenue="game play",author="",displayText="",GAME_version=version,internetVersion;
  ...
  if(menue){//when in a menue
    if(Menue.equals("creds")){//the inital loading screen 
      ...
      soundHandler.setSoundsVolume(sfxVolume);
        try{
          String inver =readFileFromGithub("https://raw.githubusercontent.com/jSdCool/CBI-games-version-checker/master/skinny_mann.txt");//check for updates
